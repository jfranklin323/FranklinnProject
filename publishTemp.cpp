/*publishTemp.cpp
 * Jasmine Franklin
 * Publishes sensor temperature date to AIO Feed
 */
...
int main(int argc, char* argv[]) {
   AnalogIn tempinput (0);
   char str_payload[100];          // Set your max message size here
   MQTTClient client;
   cout << "Starting Beagle board MQTT Adafruit Publish Example" << endl;
}

cout << "tempinput.ReadADCSample is:" << tempinput.readAdcSample() << endl;

   sprintf(str_payload, "%f", getTemperature(tempinput.readAdcSample()));
   cout<<"Payload Str is: " <<  str_payload << endl;
   ...
   cout << "Waiting for up to " << (int)(TIMEOUT/1000) <<
        ...
        " non topic " << TOPIC << " for ClientID: " << CLIENTID << endl;
